...
/**
 * A UPnP WeMo Coffee Maker.
 */
var WemoCoffee = function(device) {
	EventEmitter.call(this);
    ...
    if (this.eventService) {
        this.eventService.on("stateChange", function(value) {
            if (TRACE && DETAIL) {
                console.log("wemo coffee state change: " + JSON.stringify(value));
            }
            if (value["BinaryState"]) {
                self.emit("BinaryState", value.BinaryState);
            }
...
WemoCoffee.prototype.setBinaryState = function(value) {
	var self = this;
	var v = value ? 1 : 0;
	var args = { BinaryState : v };
	this.eventService.callAction(ACTION_SetBinaryState, args, function(err, buf) {
...
util.inherits(WemoCoffee, EventEmitter);

WemoCoffee.deviceType = "urn:Belkin:device:CoffeeMaker:1";

exports.devices = [ WemoControllee, WemoCoffee ];
exports.WemoControllee = WemoControllee;
exports.WemoCoffee = WemoCoffee;
